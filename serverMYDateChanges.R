...
function(input, output, session) {
  ...
  observeEvent(input$uploadId,{
    ...
    output$display_raw_ts <- renderUI({
      ...
      if (length(my_data) > 0 ) {
        ...
        # sidebarLayout(
        #   sidebarPanel(
        #                h4(id="big-heading","Plot Raw Data"),width=2,
  ...
  # output$display_plotRawData <- renderUI({
  #   selectizeInput(
  #     "rawDataToPlot",
  #     label = "Plot Raw Data",
  #     choices = c('a', 'b'),
  ...
  # output$display_pramsToProcess <- renderUI({
  #   selectizeInput("parameters_to_process2",label ="Select parameters to process",
  #                  choices=c('v1','v2'),
  #                  multiple = TRUE,
  #                  options = list(hideSelected = FALSE,plugins=list('remove_button'))
  #   )
  ...
  output$display_timeFormat <- renderUI({
    selectizeInput(
      "selectedTimeFormat",
      label = "Time Format",
      choices = c('Hour, Minute', 'Hour, Minute, Second','Hour in 12-hour format, Minute, AM/PM','Hour in 12-hour format, Minute, Second, AM/PM'),
      multiple = FALSE,
      options = list(
        hideSelected = FALSE,
        plugins = list('remove_button')
  ...
  output$display_timeZone <- renderUI({
    selectizeInput(
      "selectedTimeZone",
      label = "Time Zone",
      choices = c('HST', 'AKST', 'PST', 'MST', 'CST', 'EST', 'UTC'),
      multiple = FALSE,
      options = list(
        hideSelected = FALSE,
        plugins = list('remove_button')
  ...
  observeEvent(input$runQS,{
    tryCatch({
      ...
      #is valid date
      if(input$selectedDateFormat == "Year, Month, Day" && input$selectedTimeFormat == "Hour, Minute") {
          ngTest <- raw_data %>% pull(input$selectedDateFieldName)
          raw_data$date.formatted <- lubridate::ymd_hm(ngTest)
          print(head(raw_data))
      } else if(input$selectedDateFormat == "Month, Day, Year" && input$selectedTimeFormat == "Hour, Minute") {
        if(input$selectedDateFieldName == "Date.Time") {
          data.time.formatted <- mdy_hm(raw_data$Date.Time)
          raw_data$Date.Time <- as.character(data.time.formatted)
          print(head(raw_data))
        }
      }
  ...
  observeEvent(input$no_gage_downloaded,{
    shinyjs::runjs("swal.close();")
  })
  ...
  observeEvent(input$calculateDailyStatistics,{
    ...
    if(class(raw_data$Date) == "character") {
      tmpMin <- changeDateFormat(raw_data$Date, input$raw_datetime_format)
      tmpMax <- changeDateFormat(raw_data$Date, input$raw_datetime_format)
      
      dateRange$min <- min(as.Date(tmpMin))
      dateRange$max <- max(as.Date(tmpMax))
    }
  ...
  observeEvent(input$display_ts, {
     ...
     if(input$gage_id != "" && length(input$gage_id) > 0 && nrow(gageRawData$gagedata) > 0) {
      ...
      for(varName in gage_variables_to_calculate) {
          local({
                    ...
                    gage_range = calculate_time_range(as.list(gageData), input$raw_datetime_format)
    ...
    ngTest <- list()
    ngTest2 <- Reduce(full_join, myList)
  ...
  p <- ggplot(data = myData, aes(x=as.POSIXct(Date,format="%Y-%m-%d"), y = value)) +
    ...
    for(varName in variable_to_plot) {
      local({
          ...
          cal_range = calculate_time_range(myData, input$raw_datetime_format)
...
#
  ...
  calculate_time_range <- function(baseData, uploadedFormat) {
    conDate = changeDateFormat(baseData$Date,uploadedFormat)
    time_range <- difftime(max(as.POSIXct(conDate,format="%Y-%m-%d")),min(as.POSIXct(conDate,format="%Y-%m-%d")),units="days")
    if (as.numeric(time_range)<365*2){
      myBreaks = paste0(1," months")
      x_date_label = "%Y-%m-%d"
  ...
  observeEvent(input$display_gage_raw, {
    if (input$gage_id != "" && length(input$gage_id) > 0) {
      output$display_time_series_1 <- renderUI({
        withSpinner(plotlyOutput("plot_gage_ts"),
                    type = 2)
      })
    ...
    if (input$daymet_lat != "" && length(input$daymet_lat) > 0 && input$daymet_long != "" && length(input$daymet_long) > 0) {
      ...
      withProgress(message = paste("Getting DayMet data"), value = 0, {
        ...
        precipplot<-ggplot( dayMetRawData$daymetdata, aes(Date, precip))+
          ...
          scale_x_datetime(date_labels="%Y-%m-%d",date_breaks=paste0(1," months"))+
          theme_light()+
          #facet_grid(site ~., scales="fixed")+ 
          labs(x="Date", y="Precipitation (mm/day)")+
          ggtitle("Precipitation Plot")+
          theme(       
             text=element_text(size=10,face = "bold", color="cornflowerblue")
            ,plot.title = element_text(hjust=0.5)
            ,legend.position="bottom"
        ...
        output$plot_daymet_ts <- renderPlotly({
          ggplotly(precipplot) %>% plotly::layout(legend = list(orientation = "h", x = 0.4, y = -0.3))
        })
      })
    } 
  ...
  daymetStatsView <- function(dayMetDailyStats, statsVar, varToProcess) {
    ...
    daymet_range = calculate_time_range(as.list(daymetStats), "%Y-%m-%d")
    daymetBreaks = daymet_range[[1]]
    daymet_x_date_label = daymet_range[[2]]
    
    daymet_mean_col <- paste0(varToProcess,".",input$dailyStats_ts_metrics)
    
    daymetBounds <- getLowerUpperBoundsAndShading(input$dailyStats_shading, varToProcess)
    daymet_upper_col <- daymetBounds[[1]]
    daymet_lower_col <- daymetBounds[[2]]
    shading_text <- daymetBounds[[3]]
    
    if (!is.null(input$dailyStats_ts_metrics)&(input$dailyStats_ts_metrics=="mean"|input$dailyStats_ts_metrics=="median")) {
      daymet_col_selected = c("Date",daymet_mean_col,daymet_lower_col,daymet_upper_col)
      daymet_data_to_plot <- daymetStats[daymet_col_selected]
      insertUI(
        selector = "#display_time_series_3",
        where="beforeEnd",
        ui = renderPlotly({
          drawTsPlot(daymet_data_to_plot, daymet_mean_col, daymet_lower_col, daymet_upper_col, daymet_x_date_label, daymetBreaks, "DayMet" ,shading_text, NULL, daymet_data_to_plot$Date)
          #print(currentOutPutId)
        })
      )
      
    } else {
      daymet_col_selected = c("Date",daymet_mean_col)
      daymet_data_to_plot <- daymetStats[daymet_col_selected]
      insertUI(
        selector = "#display_time_series_3",
        where="beforeEnd",
        ui = renderPlotly({
         drawTsPlot(daymet_data_to_plot, daymet_mean_col, NULL, NULL, daymet_x_date_label, daymetBreaks, "DayMet",NULL, NULL, daymet_data_to_plot$Date)
          #print(currentOutPutId)
        })
      )
    }
