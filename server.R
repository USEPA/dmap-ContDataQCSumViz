...
function(input, output, session) {
  ...
  gageRawData <- reactiveValues(
    gagedata = data.frame(),
    gageColName = as.character()
  )
  dayMetRawData <- reactiveValues(
    dayMetData = data.frame(),
  ...
  getFormattedRawData <- function(dateAndTimeFields, userData, tabName, errorDivId) {
    tryCatch(
      ...
      warning = function(parsingMsg) {
        print(paste("Error while parsing", parsingMsg))
        output[[errorDivId]] <- renderUI({
          div(
            class = "alert alert-danger", id = paste0(errorDivId, "innerDiv"),
            prepareDateFormatErrorMsg(parsingMsg, tab = tabName)
          )
        })
      },
  ...
  output$displayFC <- renderUI({
    ...
    tagList(
      ...
      radioButtons("disp", "Display file information",
        choices = c(Head = "head", Tail = "tail", ColumnNames = "Column names"), inline = TRUE,
        selected = "head"
      ),
        ...
        if (localHomeDateAndTime$isTimeValid() & localHomeDateAndTime$isDateAndtimeValid()) {
          ...
          if ("date.formatted" %in% colnames(raw_data) & !is.null(localHomeDateAndTime$parmToProcess()) & nrow(raw_data) != nrow(raw_data[is.na(raw_data$date.formatted), ])) {
            ...
            output$display_actionButton_calculateDailyStatistics <-
              renderUI({
                actionButton(
                  inputId = "calculateDailyStatistics",
  ...
  observeEvent(input[["tabset"]], {
    ...
    output$summary_table_input_2 <- renderUI({
      radioButtons("summarise_by", "Summarise by",
        choices = c(
    ...
    output$summary_table_input_3 <- renderUI({
      selectizeInput("summarise_metrics",
    ...
    output$time_series_input_1 <- renderUI({
      variables_avail <- names(processed$processed_dailyStats)
      selectizeInput("dailyStats_ts_variable_name",
        label = "Select variable name",
        choices = variables_avail,
        multiple = TRUE,
        selected = variables_avail[1],
    ...
    output$time_series_input_2 <- renderUI({
      selectizeInput("dailyStats_ts_metrics",
    ...
    output$time_series_input_3 <- renderUI({
      div(
        radioButtons("dailyStats_shading", "Add shading with",
          choices = c(
    ...
    output$gage_panel <- renderUI({
      div(
        ...
        div(
          class = "panel-heading", style = "padding:10px 5px 10px 10px;",
        ...
        div(
          id = "gageVarsDiv", style = "padding:5px;display:none",
          selectizeInput("gaze_params",
    ...
    output$base_gage_daymet_panel <- renderUI({
      ...
      div(
        ...
        bsPopover(
          id = "baseDataDef", title = "What is base data\\?", content = "Base data is uploaded on the \\'Upload Data\\' tab.",
          placement = "right", trigger = "hover"
        ),
        div(
          style = "padding:5px;",
          selectizeInput("dailyStats_ts_variable_name2",
            label = "Select base variable names",
            choices = variables_avail,
            multiple = TRUE,
            selected = variables_avail[1],
            options = list(hideSelected = FALSE)
    ...
    output$daymet_panel <- renderUI({
      div(
        ...
        div(
          id = "daymetVarsDiv", style = "padding:5px;display:none",
          selectizeInput("daymet_params",
            label = "Select Daymet variables",
            choices = daymetCols,
            multiple = TRUE,
    ...
    output$time_series_overlay_input_1 <- renderUI({
      variables_avail <- names(processed$processed_dailyStats)
      selectizeInput("dailyStats_ts_overlay_variable_name",
        label = "Select variable name",
        choices = variables_avail,
        multiple = FALSE,
        selected = variables_avail[1],
    ...
    output$time_series_overlay_input_2 <- renderUI({
      selectizeInput("dailyStats_ts_overlay_metrics",
        label = "Select daily statistics metrics",
        choices = c("mean", "median", "min", "max", "range", "sd", "var", "cv", "n"),
        multiple = FALSE,
        selected = "mean",
    ...
    output$time_series_overlay_input_4 <- renderUI({
      radioButtons("overlay_shading", "Add shading with",
        choices = c(
          "none" = "none",
    ...
    output$box_input_1 <- renderUI({
      variables_avail <- names(processed$processed_dailyStats)
      selectizeInput("boxplot_variable_name",
        label = "Select variable name",
        choices = variables_avail,
        multiple = FALSE,
        selected = variables_avail[1],
    ...
    output$box_input_2 <- renderUI({
      selectizeInput("boxplot_metrics",
        label = "Select daily statistics metrics",
        choices = c("mean", "median", "min", "max", "range", "sd", "var", "cv", "n"),
        multiple = FALSE,
        selected = "mean",
    ...
    output$box_input_3 <- renderUI({
      div(
        br(),
        radioButtons("box_group", "Group by",
          choices = c(
            "month" = "month",
    ...
    output$CDF_input_1 <- renderUI({
      variables_avail <- names(processed$processed_dailyStats)
      selectizeInput("CDF_variable_name",
        label = "Select variable name",
        choices = variables_avail,
        multiple = FALSE,
        selected = variables_avail[1],
    ...
    output$CDF_input_2 <- renderUI({
      div(
        br(),
        radioButtons("CDF_shading", "Add shading with",
          choices = c(
            "25th & 75th percentiles" = "quantiles",
    ...
    output$CDF_input_3 <- renderUI({
      ...
      selectizeInput("CDF_select_year",
        label = "Select year",
    ...
    output$CDF_input_4 <- renderUI({
      selectizeInput("CDF_select_season",
        label = "Select season",
    ...
    output$raster_input_1 <- renderUI({
      variables_avail <- names(processed$processed_dailyStats)
      selectizeInput("dailyStats_raster_variable_name",
        label = "Select variable name",
        choices = variables_avail,
        multiple = FALSE,
        selected = variables_avail[1],
    ...
    output$raster_input_2 <- renderUI({
      selectizeInput("dailyStats_raster_metrics",
        label = "Select daily statistics metrics",
        choices = c("mean", "median", "min", "max", "range", "sd", "var", "cv", "n"),
        multiple = FALSE,
        selected = "mean",
    ...
    output$thermal_input_1 <- renderUI({
      ...
      selectizeInput("thermal_SiteID_name",
        label = "Select SiteID Column",
        choices = variables_avail,
        multiple = FALSE,
    ...
    output$thermal_input_3 <- renderUI({
      variables_avail <- names(uploaded_data())
      temp_keys_in_favor_order <- c(
        "Water.Temp.C", "WATER.TEMP.C", "Water_Temp_C",
      ...
      selectizeInput("thermal_Temp_name",
    ...
    output$air_vs_water_input_1 <- renderUI({
      ...
      selectizeInput("air_temp_name",
        label = "Select Air Temperature Column",
        choices = variables_avail,
        multiple = FALSE,
    ...
    output$air_vs_water_input_2 <- renderUI({
      ...
      selectizeInput("water_temp_name",
        label = "Select Water Temperature Column",
        choices = variables_avail,
        multiple = FALSE,
    ...
    output$water_temp_class_input_1 <- renderUI({
      ...
      selectizeInput("water_temp_name_in_class",
        label = "Select Water Temperature Column",
        choices = variables_avail,
        multiple = FALSE,
        selected = water_to_select,
    ...
    output$IHA_input_2 <- renderUI({
      ...
      selectizeInput("parameter_name",
        label = "Select Parameter Column",
        choices = variables_avail,
        multiple = FALSE,
      ...
      tryCatch(
        {
          ...
          if (identical(variable_to_plot, base_vars_to_plot)) {
            ...
            print("it should have updated the users")
            return(mainPlot)
          }
        },
        error = function(parsingMsg) {
    ...
    output$dateAndTimeBox <- renderUI({
      div(
        ...
        div(
          ...
          box(
            width = "100%", class = "displayed", id = "dateBox_new",
            # Need to call the dateAndTimeModule but this code is on hold so not putting time on it
            div(
              style = "margin-left:10px",
      ...
      tryCatch({
        ...
        if (nrow(new_raw_data) != nrow(new_raw_data[is.na(new_raw_data$date.formatted), ])) {
          ...
          if (!is.null(mainPlot) & length(newHomeDateAndTime$parmToProcess()) > 0 & renderStatus == FALSE) {
            ...
            output$display_time_series_new <- renderPlotly({
              ggplotly(mainPlot, height = calulatePlotHeight(length(newHomeDateAndTime$parmToProcess()))) %>% plotly::layout(legend = list(orientation = "h", x = 0.4, y = -0.4))
            })
          }
        }
        return(mainPlot)
  ...
  observeEvent(input$uploaded_overlay_newData_file, {
    ...
    # print(cols_avail)
    updateSelectizeInput(session, "overlay_newData_lower_col",
      label = "Select column to be used as lower bound",
      choices = cols_avail,
      selected = ""
    )

    updateSelectizeInput(session, "overlay_newData_longterm_col",
      label = "Select column to be used as long-term reference line",
      choices = cols_avail,
      selected = ""
    )

    updateSelectizeInput(session, "overlay_newData_upper_col",
      label = "Select column to be used as upper bound",
      choices = cols_avail,
      selected = "",
      options = list(hideSelected = FALSE)
    ...
    updateSelectizeInput(session, "overlay_newData_date_col",
      ...
      output$plot_dailyStats_ts_overlay <- renderPlotly({
        isolate({
          p1 <- ggplot(data_to_plot) +
            ...
            geom_ribbon(data = merged_overlay, aes(
              x = as.Date(yday(paste0("2000", "-", MonthDay)), "2000-01-01"),
              ymin = min, ymax = max,
              fill = "overall minimum and maximum"
            ), alpha = 0.5) +
            ...
            theme_classic() +
            theme(
              text = element_text(size = 16, face = "bold", color = "cornflowerblue"),
      ...
      output$plot_dailyStats_ts_overlay <- renderPlotly({
        isolate({
          ...
          if ((input$overlay_newData_lower_col != "") & (input$overlay_newData_upper_col != "")) {
            ...
            p1 <- p1 +
              geom_ribbon(data = data_to_add_as_overlay, aes(
                x = as.Date(yday(paste0("2000", "-", month_day)), "2000-01-01"),
                ymin = !!sym(input$overlay_newData_lower_col), ymax = !!sym(input$overlay_newData_upper_col),
                fill = input$overlay_newData_name
              ), alpha = 0.5) +
  ...
  observeEvent(input$run_CDF, {
    ...
    output$plot_CDF <- renderPlotly({
      ...
      if (is.null(CDF_plot)) {
        shinyalert("Warning", "No data available to plot for the selected variable/year/season!",
          closeOnClickOutside = TRUE, closeOnEsc = TRUE,
  ...
  observeEvent(input$display_thermal, {
    ...
    Notes.Names <- c(
      "Dataset (SiteID)", "Analysis.Date (YYYYMMDD)",
  ...
  observeEvent(input$display_thermal_sensitivity, {
    ...
    if (all(names(myList) %in% c(input$air_temp_name, input$water_temp_name))) {
      ...
      myEquation <- substitute(
        italic(y) == a + b %.% italic(x) * "," ~ ~ italic(r)^2 ~ "=" ~ r2,
        list(
          a = format(unname(coef(myModel)[1]), digits = 2),
      ...
      output$thermal_sensitivity_plot_1 <- renderPlot({
        p1 <- ggplot(data_to_plot, aes(x = !!sym(mean_col_air), y = !!sym(mean_col_water))) +
          ...
          geom_text(
            x = (min(data_to_plot[, mean_col_air], na.rm = TRUE) + 5),
  ...
  observeEvent(input$display_IHA, {
    ...
    myYr <- "calendar"
    Notes.Names <- c(
      "Dataset (SiteID)", "IHA.Year", "Analysis.Date (YYYYMMDD)",
      "Analysis.Time (HHMMSS)", "Analysis.User"
    )
  ...
  observeEvent(input$display_IHA_plot_1, {
    ...
    if (input$display_IHA_plot_1 %% 2 != 0) {
      ...
      data_for_plot_1$key <- factor(data_for_plot_1$key, levels = c(
  ...
  observeEvent(input$createReport, {
    ...
    build_summary_updated(
      dir_data = "Output/to_report/",
  ...
  prepareDateFormatErrorMsg <- function(errorMsg, tab = "") {
    ...
    if (errorMsg[1] == "All formats failed to parse. No formats found." | grepl("failed to parse", errorMsg[1], fixed = TRUE)) {
      errorMsg[1] <- "There is a mismatch between uploaded file date format and 'Selected date format', please correct and try again."
      return(errorMsg)
    } else {
      return(errorMsg)
    }
