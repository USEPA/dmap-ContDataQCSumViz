      ...
      observe({
             if(renderDataExp$render == TRUE) {
                output$time_series_input_1 <- renderUI({
                  ...
                  selectizeInput(ns("dailyStats_ts_variable_name"),label ="Select variable name",
                ...
                output$time_series_input_2 <- renderUI({
                  selectizeInput(ns("dailyStats_ts_metrics"),label ="Select daily statistics metrics",
        ...
        prepareBasePlot <- function(dataList, mapTitle, xDateLabel, xDateBrakes) {
          ...
          #write.csv(bind_rows(dataList, .id="df"),"testMean.csv",row.names=FALSE)
          mainPlot <- ggplot(data = tsData , dynamicTicks = TRUE) +
            labs(title=mapTitle, x="Date", y="Parameters")+
            geom_line(aes(x=as.POSIXct(Date,format="%Y-%m-%d"), y=upper_col), linetype = "dashed", colour = "gray")+
            geom_line(aes(x=as.POSIXct(Date,format="%Y-%m-%d"), y=value, colour=df))+
            geom_line(aes(x=as.POSIXct(Date,format="%Y-%m-%d"), y=lower_col), linetype = "dashed", colour = "gray")+
