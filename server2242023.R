...
function(input, output, session) {
  ...
  observeEvent(input$uploadId,{
    ...
    output$display_raw_ts <- renderUI({
      ...
      if (length(my_data) > 0 ) {
        ...
        # idx_ID_col <- str_detect(my_colnames, regex('ID|SITE|FLAG|Comment', ignore_case = T))
        # print(my_colnames[idx_ID_col])
  ...
  observeEvent(input$showrawTS,{
    ...
    if (validateUserInputs(raw_data) == FALSE){
        ...
        if (!is.null(my_raw_choices) && !is.null(raw_data$date.formatted)){
          ...
          p <- ggplot(data = uploaded_raw_data, aes(x = as.POSIXct(date.formatted,format="%Y-%m-%d"), y = value)) +
  ...
  observeEvent(input$display_ts, {
         ...
         if (!is.null(input$dailyStats_ts_metrics)&(input$dailyStats_ts_metrics=="mean"|input$dailyStats_ts_metrics=="median")&input$dailyStats_shading!="newData"){
          ...
          overridePotlyStyle("plot_gage_ts")
          overridePotlyStyle("plot_daymet_ts")
  ...
  overridePotlyStyle <- function(elementId) {
    ...
    temStr <- paste("$(",elementId,".removeAttr('style')", sep="")
    shinyjs::runjs(temStr)
  }
